...
function getPreproductionDetailAgainstId($con, $prehid)
{
    $detail_stockin_hand = $detail_price = $detail_prodid = $detail_qty = $detail_uom = [];
    $all_uom_array = getProductUomArray ($con);
    $result = mysqli_query ($con, "SELECT *, pd.prodid as detail_prodid ,pd.stockinhand as detail_stockinhand , pd.uom as detail_uom FROM `tblpreproductionhead` ph join tblpreproductiondetail pd on pd.prehid = ph.prehid where ph.prehid=$prehid");
